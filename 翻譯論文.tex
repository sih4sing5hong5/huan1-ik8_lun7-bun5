...
語料格式的影響著翻譯的效果，除了斷字，翻譯嘛會使用斷詞\footnote{斷詞的定義請看\ref{節：斷詞方法}}做單位，斷詞的分數比斷詞組閣較好一寡，毋過小較輸斷字，是76.88分。三个分數整理佇表\ref{表：斷詞組、斷詞、斷字做單位的翻譯分數}。

\begin{table}
\caption{斷詞組、斷詞、斷字做單位的翻譯分數}%
\label{表：斷詞組、斷詞、斷字做單位的翻譯分數}
\begin{tabular}{llll}
 & 斷詞組 & 斷詞 & 斷字\\
BLEU分數　& 70.67 & 76.88 & 82.94\\
\end{tabular}
\end{table}
