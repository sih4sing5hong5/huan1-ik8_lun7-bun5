...
\section{翻譯架構}
\label{節：翻譯架構}
國語翻譯到臺語需要兩項物件，一項是國語翻譯到臺語的語詞對照表，另外一項是知影臺語逐句好歹的語言模型。這馬時行的翻譯系統是統計翻譯（），會當分做三个部份：
...
翻譯大部份攏用BLEU（Bilingual Evaluation Understudy）來評分\footnote{程式佇：\url{https://github.com/moses-smt/mosesdecoder/blob/master/scripts/generic/multi-bleu.perl}}，伊用連紲詞的概念來評分，公式會當看式。

$BLEU=100\times{e^{\max{0,a\div{z}}}\times{}}$
